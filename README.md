...
牢 A (Anthropic) 官方 [Claude Code](https://docs.anthropic.com/en/docs/claude-code) CLI 工具的源码反编译/逆向还原项目。目标是将 Rain Code 大部分功能及工程化能力复现 (问就是老佛爷已经付过钱了)。虽然很难绷, 但是它叫做 CCB(踩踩背)...
...
从 `@rainhole/rain-code@1.0.2` 开始，这个包只安装 `rain-code` 命令，不再附带 `ccb` 和 `claude-code-best` 别名。
...
- ⚙️ 常规的配置 Rain Code 的方式, 各大提供商都有自己的配置方式
